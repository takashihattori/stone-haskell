...
parseStatement (TokenID _ "ほむら":ts1) =
  do (tree, ts2) <- parseBlock ts1
     return (WhileState (ASLeaf (TokenNum 0 1)) tree, ts2)
parseStatement (TokenID _ "まどか":ts) =
  return (BreakState, ts)
parseStatement (TokenID _ "ワルプルギスの夜":ts) =
  return (ContState, ts)
